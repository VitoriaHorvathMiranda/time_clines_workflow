...
rule separate_time_pops:
   ...
   output: expand(os.path.join(config['call_path'], "TIME_SEP_POPs/TimeSEP_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_{year}.tsv"), year=config['CLINAL_YEAR'])
   wildcard_constraints: year= "|".join(config['CLINAL_YEAR'])
   params:
        path =  os.path.join(config['call_path'], "TIME_SEP_POPs/TimeSEP_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_")
...
rule separete_chrom:
    input: os.path.join(config['call_path'], "TIME_SEP_POPs/TimeSEP_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_{year}.tsv")
    output: temp(os.path.join(config['call_path'], "TIME_SEP_POPs/TimeSEP_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_{year}_{chrom}.tsv"))
...
rule make_glm_plots:
    input: 
        glm = lambda wildcards: [
            os.path.join(
                config['analysis_path'], 
                f"time_GLM_lat/p-values_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_{wildcards.clinal_year}_{chrom}.tsv"
            )
            for chrom in config['chrom']
        ],
    output: 
        man = "../results/manhattan_{clinal_year}.jpeg",
        ht = "../results/P-values_hist_{clinal_year}.jpeg",
        qvalues = os.path.join(config['analysis_path'], "time_GLM_lat/q-values_noSNC10_noESC97_with_dlGA10_dlSC10_mincount5_minfreq0.001_cov15_{clinal_year}.tsv"),
        qhists = "../results/hist_p-values_withPi_{clinal_year}.jpeg",
        qPlot = "../results/q_Plot_{clinal_year}.jpeg",
    
    wildcard_constraints: chrom = "|".join(config['chrom']), year= "|".join(config['CLINAL_YEAR']), 
